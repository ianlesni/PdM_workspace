...
#define LED_BLINK_PATTERNS		3		// Cantidad de patrones de bink disponibles
#define LED_ON_TIME_1_MS		1000
#define LED_ON_TIME_2_MS		200
#define LED_ON_TIME_3_MS		100
/* USER CODE END PD */
...
/* USER CODE BEGIN 0 */
delay_t blinkDelay;
tick_t blinkDurations[LED_BLINK_PATTERNS] = {LED_ON_TIME_1_MS, LED_ON_TIME_2_MS, LED_ON_TIME_3_MS};
uint8_t toggleCount;
uint8_t durationIndex;
/* USER CODE END 0 */
...
int main(void)
{
  ...
  MX_GPIO_Init();
  /* USER CODE BEGIN 2 */
  delayInit(&blinkDelay, blinkDurations[0]);
  toggleCount = 0;
  durationIndex = 0;
  /* USER CODE END 2 */
    ...
    /* USER CODE BEGIN 3 */
	  if (delayRead(&blinkDelay))
	  {
		  ...
		  if (toggleCount >= LED_BLINK_CYCLES*2)
		  {
			  toggleCount = 0;
			  durationIndex ++;
			  if (durationIndex >= LED_BLINK_PATTERNS)
			  {
				  durationIndex = 0;
			  }
			  delayWrite(&blinkDelay, blinkDurations[durationIndex]);
		  }
	  }
  }
  /* USER CODE END 3 */
...
/**
  ...
  * @retval ture Transcurrió el tiempo configurado.
